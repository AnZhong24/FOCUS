...
class SDARAttention(nn.Module):
    ...
    def forward(
        ...
        **kwargs: Unpack[FlashAttentionKwargs],
        ...
        # Check if we're in decode mode and need to compute importance
        context = get_context()
        ...
        if token_evicting:
            ...
            # Filter query and key states to only include masked tokens
            masked_query_states = query_states[:, :, seq_mask, :]  # [batch, heads, masked_seq_len, head_dim]
            masked_key_states = key_states[:, :, seq_mask, :]      # [batch, heads, masked_seq_len, head_dim]
            
            # Handle Grouped Query Attention (GQA) by repeating key states
            num_key_value_groups = self.num_attention_heads // self.num_key_value_heads
            masked_key_states = repeat_kv(masked_key_states, num_key_value_groups)
            
            # Calculate attention scores only among masked tokens
            attention_scores = torch.matmul(masked_query_states, masked_key_states.transpose(-2, -1)) * self.scaling
            ...
            attention_sum = attention_weights.sum(dim=-2)  # Shape: [batch_size, num_heads, seq_len_filtered]
            importance_scores = attention_sum.sum(dim=-2)  # Shape: [batch_size, seq_len_filtered]
            
            if self.layer_idx == 0:
                # Store importance scores in context for layer 0
                set_context(is_decode=context.is_decode, enable_token_eviction=True, first_layer_importance=importance_scores)
            else:
                ...
                if context.strategy == "dynamic":
                    ...
                    # Select bottom K tokens with lowest delta
                    sorted_indices = torch.argsort(delta_importance, stable=True, descending=False)
                    retain_indices = sorted_indices[0, :context.K]
        ...
        return attn_output, None, processing_mask # , attn_weights
...
class SDARDecoderLayer(GradientCheckpointingLayer):
    ...
    def forward(
        ...
        **kwargs: Unpack[FlashAttentionKwargs],
        ...
        # Self Attention
        hidden_states, self_attn_weights, processing_mask = self.self_attn(
            hidden_states=hidden_states,
        ...
        context = get_context()
        if self.self_attn.layer_idx == 1 and context.enable_token_eviction and context.is_decode and context.mask_indices.sum() > context.K:
    ...
    def forward(
        ...
        **flash_attn_kwargs: Unpack[FlashAttentionKwargs],
        ...
        if context.is_decode:
            hidden_states = inputs_embeds[:, context.processing_indices]
            position_ids = original_position_ids[:, context.processing_indices]
            attention_mask = original_attention_mask[:, context.processing_indices]
        ...
        for i in range(self.config.num_hidden_layers):
            ...
            if i == 1 and context.enable_token_eviction and context.is_decode and context.mask_indices.sum() > context.K:
    ...
    def forward(
        ...
        **kwargs: Unpack[KwargsForCausalLM],
        ...
        return CausalLMOutputWithPastAndBlockCache(
            ...
            block_key_values=outputs.block_key_values,
        )
