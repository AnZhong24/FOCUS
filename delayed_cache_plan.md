...
- `focus_generate.py`: orchestration of delayed cache passes and context updates (`uncached_positions`, `processing_indices`, etc.).
...
3. **Attention Metadata Extensions**
   - Avoid the external `context.py`; instead, extend the attention metadata carried in `StepContext.attn_metadata` to transport delayed-cache fields (ragged `processing_indices`, ragged `q_seqlens`, `use_block_cache`, etc.).
   - Align `processing_indices` with `q_start_loc`: tokens belonging to sequence *i* live in the slice `[q_start_loc[i], q_start_loc[i] + q_seqlens[i])` of both tensors so Triton kernels can compute pointer math without extra bookkeeping.
   - Ensure metadata defaults collapse to “full block” when delayed cache is disabled so existing kernels remain unaffected; in that case we avoid allocating or threading `processing_indices` entirely.
...
6. **Model Agent Orchestration**
   - Augment `DLLMModelAgentStrategy` (`model_agent.py`) to:
     1. Derive `uncached_positions` and `processing_indices` from sequence state for denoising steps.
     2. If the block is fully unmaksed, the `processing_indices` should be set to the entire block to flush KV states.
     ...
     4. At the tail of each decode pass, call into the sequence helpers added in Step 2 to (a) mark any tokens that became stable during this pass and (b) regenerate the ragged metadata for the next pass. This ensures the second through Nth passes only touch the newly unstable indices instead of reprocessing the entire block.
...
7. **Model Runtime Support & Kernel Selection**
   ...
   - Introduce sparse-aware copies of the Triton `fill_kv_cache` and `paged_attention` kernels that accept explicit processing indices plus ragged `q_start_loc`/`q_seqlens`. Keep the existing dense kernels untouched and use them whenever delayed cache is disabled.
...
10. **Sparse Kernel Semantics & Output Scatter**
   - `fill_kv_cache_sparse`: consume ragged `q_start_loc`/`q_seqlens` together with `processing_indices`, compute the target offset inside each block (history offset + index), and scatter the key/value rows into the correct cache slots. The kernel only touches the `processing_indices` subset so other positions remain untouched.
     - Inputs: same tensor layout as dense kernel plus `processing_indices` (flattened), `q_start_loc`, `q_seqlens`. Each CTA gathers token offsets directly from the flattened index list so we avoid touching unused cache slots.
   ...
   - `paged_attention_sparse`: read queries in ragged form (similar to the varlen flow in `lmdeploy/pytorch/backends/cuda/flash_attention.py`). Use the same ragged metadata to decide how many queries belong to each request without assuming contiguous blocks.
...
11. **After forward (Logit Assembly)**
   - `SDARForCausalLM.get_logits` expects contiguous `[tokens, hidden]` inputs, and the DLLM `UnmaskingProcessor` (`unmasking.py`) indexes logits by block layout (`block_len` stride). When delayed cache is enabled, run the LM head over the ragged hidden states ordered by `processing_indices`, and scatter the resulting logits into a dense `[batch, block_len, vocab]` tensor before calling the unmasker.
